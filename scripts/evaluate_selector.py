"""Frozen synthetic full-selector evaluation; never trains or changes the selector."""
import json
import hashlib
import os
import statistics
...
def evaluate(call):
    ...
    for case in cases:
        ...
        rows.append({'id':case['id'],'correct':correct,'status':result['status'],'elapsed_ms':round(elapsed,3),
                     'plan':plan,'expected_selection':case['expected'] is not None,'category':case.get('category','legacy_development'),
                     'vita_supported':case.get('vita_supported',True),'reason':result.get('reason')})
    return {'selector_sha256':result['selector_sha256'],'model_revision':result.get('model_revision'),
